...
const DEFAULT_TERMS = `1. 本報價僅供參考，所有尺寸、設計及細節需經雙方確認後方為最終訂單依據。
2. 客戶需自行量度展示物品尺寸，本公司提供之尺寸建議（包括按經驗額外預留約5–10cm空間）僅供參考用途，最終尺寸必須由客戶確認，本公司不會就尺寸不合適承擔責任。
3. 所有產品均為訂製產品，客戶確認訂單後，恕不接受取消或退款；如需更改尺寸、設計或配件，可能需重新報價及安排製作時間。
4. 本報價中所列之優惠或折扣（如適用）只限於指定期間內有效，逾期將不再適用，本公司保留最終決定權。
5. 產品完成後將安排送貨，實際送貨日期需視乎生產進度及物流安排而定。
6. 如因送貨地點之環境限制（包括但不限於門口尺寸、樓梯、電梯或通道空間等）導致無法順利送貨或需額外安排，本公司恕不承擔相關責任或費用。
7. 客戶需於收貨時即時檢查產品，如發現明顯損壞或問題，請即時提出，否則視為驗收完成。
8. 如產品因不當使用、人為損壞或自行安裝不當而造成損壞，本公司概不負責。
